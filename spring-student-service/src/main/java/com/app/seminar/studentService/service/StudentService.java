...
public class StudentService {
	...
	public String getOffers(String courseNames, String state) {
		...
		String commaSeparatedCourses = registrations.stream()
				.filter(s-> courseNames.contains(s.getCourseName()))
				.map(Student::getCourseName).distinct().collect(Collectors.joining(","));

		//now we just considered courses for todays registrations in commaSeparatedCourses
